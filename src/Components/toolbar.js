 ...
 useEffect(() => {
  ...
  const fetchData = async () => {
    if (type === "study") {
      ...
      const objArrayAsArray = Array.isArray(objArray) ? objArray : [objArray];
      objArrayAsArray.forEach((objPath) => {
        ...
        thumbnails.push({
          ...
          seriesNumber: 1,
          numImageFrames: null,
          stackPercentComplete: objPath.stackPercentComplete || 0,
      ...
      Series.forEach(async (s) => {
        ...
        thumbnails.push({
          ...
          seriesNumber: 1,
          numImageFrames: null,
          stackPercentComplete: obj.stackPercentComplete || 0,
  ...
  useEffect(() => {
    const element = document.getElementById('Viewer-Container'); // Replace with your viewport ID
    cornerstone.enable(element);
    return () => {
        cornerstone.disable(element); // Cleanup on unmount
    };
}, []);

  const handleSelectTool = (tool) => {
    // if (tool === "ResetViewport") {
    //   resetViewport();
    // }
    ...
    //   if (element && element.cornerstoneElement) {
    //     cornerstone.reset(element);
    //   } else {
    //     console.error("Element is not enabled. Please enable it first.");
    //   }
    // }
  };
  ...
  const resetViewport = () => {
    const element = document.getElementById('Viewer-Container'); // Replace with your element ID
    if (!cornerstone.getEnabledElement(element)) {
      console.error('Viewport element is not enabled.');
      return;
    }
  ...
  const handleResetViewport = () => {
    setState((prevState) => ({
      ...
      pan: { x: 0, y: 0 }, // Reset pan to center
      activeTool: null, // Deactivate any active tool
      rotation: 0, // Reset rotation
    }));
  
    // Additional reset logic for other properties, if any
